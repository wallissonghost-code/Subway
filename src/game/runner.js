...
export function updateRunner(dt){
 ...
 for(const o of near.obstacles){const z=o.z+near.worldZ;if(Math.abs(z)<=Math.max(.38,o.radius||0)&&o.lane===state.targetLane){if(o.type==='jump'&&state.y>.78)continue;crash();break}}
 onUpdate(state);
}
function crash(){state.alive=false;state.mode='gameover';state.score=Math.floor(state.score);onUpdate(state);onGameOver(state)}
export function getRunnerSnapshot(){return{mode:state.mode,paused:state.paused,alive:state.alive,lane:state.targetLane,y:state.y,sliding:state.sliding,speed:state.speed,distance:state.distance,score:state.score,coins:state.coins,multiplier:state.multiplier}}

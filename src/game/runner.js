...
export function updateRunner(dt){
 ...
 if(!qaCollisionBypass){for(const o of near.obstacles){const z=o.z+near.worldZ;if(Math.abs(z)<=Math.max(.38,o.radius||0)&&o.lane===state.targetLane){if(o.type==='ramp')continue;if(o.type==='jump'&&state.y>.78)continue;if(o.type==='block'&&near.surfaceY>.9)continue;crash();break}}}
 onUpdate(state);
}
